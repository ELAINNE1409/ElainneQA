  ...
  public async execute(req: UpvotePostDTO): Promise<UpvotePostResponse> {
    ...
    try {
      ...
      if (member.memberId.equals(post.memberId)) {
        return left(new UpvotePostErrors.PostOwnerCanNotVoteError(req.userId));
      }
      ...
      for (let i = 0; i < existingVotesOnPostByMember.length; i++) {
        if (existingVotesOnPostByMember[i].isUpvote()) {
          return left(new UpvotePostErrors.AlreadyUpvotedError(req.slug, req.userId));
        }
      }
