...
> {
  ...
  isFormValid(): boolean {
    ...
    if (
      ...
      TextUtil.atLeast(
        newCommentText.replace(/(<([^>]+)>)/gi, ''),
        CommentUtil.minCommentLength
      ) ||
      TextUtil.atMost(
        newCommentText.replace(/(<([^>]+)>)/gi, ''),
        CommentUtil.maxCommentLength
      )
    ) {
      toast.error(
        `Yeahhhhh, comments should be ${CommentUtil.minCommentLength} to ${
          CommentUtil.maxCommentLength
        } characters. Yours was ${
          newCommentText.replace(/(<([^>]+)>)/gi, '').length
        }. 🤠`,
        {
          autoClose: 3000
        }
  ...
  onSubmitComment() {
    //console.log(this.state.newCommentText.replace(/(<([^>]+)>)/gi, ''));
    if (this.isFormValid()) {
