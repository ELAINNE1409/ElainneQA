...
class CommentPage extends React.Component<CommentPageProps, CommentState> {
  ...
  isFormValid () : boolean {
    ...
    if (!!newCommentText === false ||
      TextUtil.atLeast(newCommentText.replace(/(<([^>]+)>)/gi, ''), CommentUtil.minCommentLength) ||
      TextUtil.atMost(newCommentText.replace(/(<([^>]+)>)/gi, ''), CommentUtil.maxCommentLength)
    ) {
      toast.error(`Yeahhhhh, comments should be ${CommentUtil.minCommentLength} to ${CommentUtil.maxCommentLength} characters. Yours was ${newCommentText.replace(/(<([^>]+)>)/gi, '').length}. 🤠`, {
        autoClose: 3000
      })
      return false;
    }
