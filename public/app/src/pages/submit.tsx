...
class SubmitPage extends React.Component<SubmitPageProps, SubmitPageState> {
  ...
  isFormValid (): boolean {
    ...
    if (postType === 'text') {
      const clearedText = text.replace(/<\/?p>/g, '');
      if (!textPresent || 
        TextUtil.atLeast(clearedText, PostUtil.minTextLength) || 
        TextUtil.atMost(clearedText, PostUtil.maxTextLength)
      ) {
        //toast.error(text);
        //toast.error(title);
        //toast.error(link);
        //toast.error(clearedText);
        
        toast.error(`Yeahhhhh, text posts should be ${PostUtil.minTextLength} to ${PostUtil.maxTextLength} characters. Yours was ${clearedText.length}. 🤠`, {
          autoClose: 3000
        })
        return false;
      }
      ...
      if (!linkPresent || 
        ...
        toast.error(`Yeahhhhh, link posts should be ${PostUtil.minLinkLength} to ${PostUtil.maxLinkLength} characters. Yours was ${link.length}. 🤠`, {
          autoClose: 3000
        })
        return false;
      } 
